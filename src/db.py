...
class Database:
    ...
    async def is_channel_in_database(self, channel_name: str) -> bool:
        async with self.db_cursor() as cursor:
            await cursor.execute("SELECT COUNT(*) FROM channels WHERE channel_name = ?", (channel_name,))
        ...
        async with self.db_cursor() as cursor:
            for record in records:
                await cursor.execute(
                    ...
                    (record.id, record.channel_url, record.username, record.title, record.participants_count,
                     record.date, record.scam,
                     record.has_link, record.fake)
                )

    async def insert_document_blob(self, message_id: int, channel_id: int, channel_username: str, file_name, mime_type,
                                   file_blob: bytes) -> None:
        async with self.db_cursor() as cursor:
            await cursor.execute("SELECT 1 FROM documents WHERE message_id = ?", (message_id,))
            if await cursor.fetchone() is None:
                await cursor.execute(
                    "INSERT INTO documents (message_id, channel_id,  channel_name,file_name,mime_type, file_blob) VALUES (?, ?, ?, ?,?,?)",
                    (message_id, channel_id, channel_username, file_name, mime_type, file_blob))
    ...
    async def update_last_processed_message_id(self, channel_name: str, message_id: int) -> None:
        async with self.db_cursor() as cursor:
            await cursor.execute(
    ...
    async def save_image_blob(self, channel_id: int, channel_username: str, message_id: int, photo_id: int,
                              image_data: bytes):
    ...
    async def save_reactions(self, message_id: int, channel_id: int, channel_username: str, emoticon, count: int):
        async with self.db_cursor() as cursor:
