...
from src.app import init_telegram_client, process_channel, download_messages, download_document, \
    download_large_media
...
async def main():
    client = await init_telegram_client("snooper", os.getenv("PHONE"), int(os.getenv("API_ID")),
                                        os.getenv("API_HASH"))
    ...
    try:
        for channel in channel_list:
            ...
            logger.info("Last message in db is %s, from channel > %s", last_message_id_in_db, from_channel)
            ...
            if last_message_id_in_db > 0:
                ...
                logger.info("Last message_id %s in our db, last message id in channel %s channel %s",
